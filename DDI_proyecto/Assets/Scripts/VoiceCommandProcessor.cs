...
namespace IBM.Watsson.Examples
{
    public class VoiceCommandProcessor : MonoBehaviour
    {
        ...
        /*Yo lo agregue*/
        public delegate void OnCommandRecognized(string command);
        public OnCommandRecognized onCommandRecognized;

        void Start()
        {
            LogSystem.InstallDefaultReactors();
            Runnable.Run(CreateService());
            Debug.Log("VoiceCommandProcessor iniciado...");
        }
            ...
            if (result != null && result.results.Length > 0)
            {
                foreach (var res in result.results)
                {
                    foreach (var alt in res.alternatives)
                    {
                        ...
                        /*Solo quiero la respuesta final, no los intermedios*/
                        if(res.final)
                        {
                            Debug.Log($"Respuesta final: {alt.transcript}");    /*En el JSON exiten un espacio extra en la cadena sin importar que sea final*/
                            if(commands.Contains(alt.transcript) || (alt.transcript.Contains("intensity number ")))       
                            {
                                Debug.Log($"Comando detectado con exito!: {alt.transcript}");
                                onCommandRecognized?.Invoke(alt.transcript); // ?  == es para comparar si es null
                                break;
                            }
                        }
                    }
