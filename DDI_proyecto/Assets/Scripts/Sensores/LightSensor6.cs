...
public class LightSensor6 : MonoBehaviour
{
    /*Configuración del cliente*/
    public string brokerEndpoint = "test.mosquitto.org";
    public string lightTopic = "light6";
    ...
    void Update ()
    {
        ...
        if((reportTimer += Time.deltaTime) >= reportRate){
            ...
            Debug.Log($"[LightSensor-6]Sending report topic: {lightTopic}, Status: {message}...");
    ...
    public void OnLightStateChange(bool state, int id)
    {
        if(id == 10 || id == 11)
        {
            Debug.Log($"[LightSensor-6] Comando recibido, estado={state}, id={id}");
    ...
    public void OnIntensityLightChange(int intensity, int id)
    {
        ...
        if(id == 6)
        {
            Debug.Log($"[LightSensor-6] Comando recibido, intesidad={intensity}, id={id}");
    ...
    public void ToggleLight(){   /*Cambiar el valor en el sensor, para que este le diga al subscritor que apague o prenda la luz*/
        Debug.Log("[LightSensor-6] Se hizo toogle");
