...
public class LightSensor7 : MonoBehaviour
{
    /*Configuración del cliente*/
    public string brokerEndpoint = "test.mosquitto.org";
    public string lightTopic = "light7";
    ...
    void Update ()
    {
        if(!client.IsConnected){
            Debug.LogWarning("Sensor 6: No conectado");
            return;
        }

        if((reportTimer += Time.deltaTime) >= reportRate){
            String message = lightState.ToString() + "," + lightIntensity.ToString();
            Debug.Log($"[LightSensor-7]Sending report topic: {lightTopic}, Status: {message}...");
    ...
    public void OnLightStateChange(bool state, int id)
    {
        if(id == 12 || id == 13)
        {
            Debug.Log($"[LightSensor-7] Comando recibido, estado={state}, id={id}");
    ...
    public void OnIntensityLightChange(int intensity, int id)
    {
        ...
        if(id == 7)
        {
            Debug.Log($"[LightSensor-7] Comando recibido, intesidad={intensity}, id={id}");
    ...
    public void ToggleLight(){   /*Cambiar el valor en el sensor, para que este le diga al subscritor que apague o prenda la luz*/
        Debug.Log("[LightSensor-7] Se hizo toogle");
